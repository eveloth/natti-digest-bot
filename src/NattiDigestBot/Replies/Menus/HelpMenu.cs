    ...
    public static Reply CategorySection { get; } =
        new()
        {
            ReplyText =
                ...
                + "/new_category <i>ключ - описание - порядок отображения</i>\n\n"
                + "например:\n/new_category котики - Фотографии ваших котиков - 1\n\n"
                + "Порядок отображения — это порядковый номер, в соответствии с которым категория "
                + "будет отображаться в дайджесте. Его можно оставить пустым — категории без порядка "
                + "будут идти в самом низу, а категории с указанным порядком будут отсортированы по нему.\n\n"
                ...
                + "/update_category <i>111 - коооотики - очень милые КОТИКИ - 1</i>.\n"
                + "Если при редактировании категории ты не укажешь порядок отображения, то он сбросится — "
                + "в том случае, конечно, если он вообще был указан.\n\n"
    ...
    public static Reply DigestSection { get; } =
        new()
        {
            ReplyText =
                ...
                + "Уже добавленное сообщение можно изменить: переместить в другую категорию, "
                + "поменять описание или ссылку. Чтобы сделать это, узнай ID сообщения и дай следующую команду:\n\n"
                + "/edit <i>111\n"
                + "новая категория\n"
                + "новое описание\n"
                + "новая ссылка</i>\n\n"
